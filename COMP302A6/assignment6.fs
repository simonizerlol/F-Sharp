...
(* Question 1 *)
//compute the whole possibly infinite sequence of digits obtained.
//val expand : num:int -> den:int -> radix:int -> seq<int>
let rec expand num den radix = 
     Seq.delay(fun () -> if ((num*radix) < den ) 
                         then cons 0 (expand (num*radix) den radix)
                         else cons ((num*radix)/den) (expand ((num*radix)%den) den radix))
...
//computes the indefinite integral of a term symbolically using the standard formula
//integral ( cx^n dx =  (n*c)x^(n+1))
let integrateTerm (t: term):term = 
    match t with
    | Term (c,e) -> Term(c/(float(e+1)),e+1)
(* Just 2 lines of code! *)

//We represent a power series as an infinite stream of terms.
//computes the integral of such a stream lazily producing an infinite stream
//val integrateSeries : sigma:seq<term> -> seq<term>
let rec integrateSeries sigma = 
    let x = integrateTerm (first sigma)
    Seq.delay (fun() -> cons x (integrateSeries (rest sigma)))
(* Just 2 lines of code! *)
...
(*
Using (co)-recursion and the second equation above for the exponential series, 
create an infinite stream representation of the exponential series.
*)
let rec expSeries = 
    Seq.delay (fun() -> cons (Term(1.0, 0)) (integrateSeries expSeries))

(* Just one line!  Do NOT forget to delay the code. *)

//computes the sum of the first n terms of a power series evaluated at x.
